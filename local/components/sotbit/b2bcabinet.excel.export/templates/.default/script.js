(function () {
    ...
    window.JCB2BExcelExport = function (params) {
        ...
        this.modeWork = params.model_of_work;
        this.filter = params.filter;
        this.cond_tree_params = params.cond_tree_params;
        this.modalCondTree = document.querySelector(params.modalSelector);

        if (this.btnExport) {
            if (this.modeWork === 'default') {
                BX.bind(this.btnExport, 'click', BX.delegate(this.exportModeDefaultAction, this));
            }
        }

        if (this.modalCondTree) {
            document.body.appendChild(this.modalCondTree);
        }

        if (this.cond_tree_params.SEND_BTN_NAME) {
            BX.bind(document.querySelector('[name="' + this.cond_tree_params.SEND_BTN_NAME + '"]'), 'click', BX.delegate(this.exportModeUserFilterAction, this));
        }
    };

    window.JCB2BExcelExport.prototype =
        {
            exportModeUserFilterAction: function () {
                var condForm = document.querySelector('[name="' + this.cond_tree_params.FORM_NAME + '"]');
                if (condForm) {
                    this.filter =  BX.ajax.prepareForm(condForm);
                    this.exportModeDefaultAction();
                }
            },
            exportModeDefaultAction: function() {

                this.setIcon("icon-spinner2 spinner mr-2");
            ...
            setDefaultIcon: function () {
                var iconContainer = this.btnExport.querySelector("i");
                iconContainer.setAttribute("class", "icon-upload mr-2");
            },
