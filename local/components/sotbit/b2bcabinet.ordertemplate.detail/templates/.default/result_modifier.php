...
if ($arResult["PRODUCTS"]) {
    ...
    foreach ($arResult["PRODUCTS"] as $key => $product) {
        ...
        if ($product["PICTURE"]["SRC"]) {
            ...
            $img = $this->GetFolder() . "/img/no_photo.png";
        }
        if ((!$product["PRICE"] && !$product["PRICE_AVAILABLE"]) || $product["ACTIVE"] == "N" ) {
            $unavailableCount++;
            $unavailable[$unavailableCount]["data"]["ID"] = $product["PRODUCT_ID"];
            $unavailable[$unavailableCount]["data"]["NAME"] = '<div class="orders-templates__image-container"><img src="' . $img . '"></div><div class="orders-templates__product-link"><a href="' . $product["DETAIL_PAGE_URL"] . '" target="_blank">' . $product["NAME"] . '</a></div>';
            ...
            $available[$availableCount]["data"]["NAME"] = '<div class="orders-templates__image-container"><img src="' . $img . '"></div><div class="orders-templates__product-link"><a href="' . $product["DETAIL_PAGE_URL"] . '" target="_blank">' . $product["NAME"] . '</a></div>';
