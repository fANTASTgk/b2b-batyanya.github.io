...
class SotbitProfileAdd extends CBitrixComponent
{
	...
	{
		$this->checkProps($request);
		if(empty($this->arResult['ERRORS']))
		{
			$idProfile = $this->executeAddProperties($request);
		}
	 ...
	 * @return void
	 */
	protected function executeAddProperties($request) {
		...
		if(empty($this->arResult['ERRORS'])) {
			if(false && Loader::includeModule('sotbit.auth') && Option::get('sotbit.auth', 'CONFIRM_BUYER', 'N', SITE_ID) == 'Y')
			{
				...
				while ($orderProperty = $orderPropertiesList->fetch()) {
				    if(is_array($value = $request->get('ORDER_PROP_' . $orderProperty['ID']))){
				        $value = implode(",", $value);
                    }
