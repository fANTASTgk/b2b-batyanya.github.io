    ...
    foreach ($arResult["TEMPLATES"] as $key=> $template){
        ...
        if($template["SAVED"] != "N"){
            ...
            $arResult["ROWS"][$key]['actions'][] = [
                ...
                "ONCLICK" => "exportExcelTemplate(".$template["ID"].")",
                "DEFAULT" => "1"
            ];
        }
        ...
        if($template["USER_ID"] == $USER->GetID()){
            $arResult["ROWS"][$key]['actions'][] = [
                "TEXT" => GetMessage("SOTBIT_B2BCABINET_ORDERTEMPLATES_ACTION_EDIT"),
                "ONCLICK" => "jsUtils.Redirect(arguments, '".$pathDetail."&edit=Y')",
                "DEFAULT" => true
            ];
