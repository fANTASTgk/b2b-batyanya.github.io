;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		sendRequest: function(action, data)
		{
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(result) {
					...
					if(this.multibasket.initParams.params.HIDE_ON_BASKET_PAGES !== 'Y'){
						const checkBoxes = [];
						this.getItemsToEdit().forEach(item => {
							checkBoxes.push(this.getCacheNode(this.ids.basketRoot).querySelector(`[data-id="${item}"] [data-entity="baseket-item-checkbox"]`));
						});
		...
		{
			const colorConteiner = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'multubasket-color')
			let color;
			if (this.multibasket.initParams.params.HIDE_ON_BASKET_PAGES === 'Y') {
				color = this.multibasket.initParams.result.BASKETS.filter(i => i.CURRENT_BASKET)[0].COLOR;
			} else {
				color = this.multibasket.instance.baskets.filter(i => i.CURRENT_BASKET)[0].COLOR;
			}

			if (colorConteiner instanceof HTMLDivElement) {
				colorConteiner.style = `background-color: #${color}`;
			}

			if(this.multibasket.initParams.params.HIDE_ON_BASKET_PAGES !== 'Y'){
				const checkboxs = this.getEntities(this.getCacheNode(this.ids.basketRoot), 'baseket-item-checkbox')
				Array.prototype.slice.call(checkboxs).forEach(function(i) {
					i.style = `border-color: #${color};`
					const birdie = i.querySelector('polyline');
					const style = birdie.getAttribute('style').replace('###', `#${color}`);
					birdie.setAttribute('style', style);
				});
			}
		},
		...
		initMultibasket: function()
		{
			...
			this.multibaksetTitle = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'multibasket__title');
			this.multibaksetListWrapper = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'multibasket__otherbasket_wraper');

			BX.addCustomEvent(
				window,
				'sotbitMultibasketInitialized',
				function() {
					this.multibasket.instance.setAdditionsBasketListRoot(this.multibaksetListWrapper);
					this.setColorFromMultibasket();
				}.bind(this),
			);

			if(this.multibasket.initParams.params.HIDE_ON_BASKET_PAGES === 'Y'){
				this.multibaksetTitle.style.display = 'none';
				return;
			}
			this.multibaksetTitle.addEventListener('click', function() {

				const activeClass = 'multibasket__title-active';
				if (!this.multibaksetTitle.classList.contains(activeClass)) {
					return;
				}

				if (this.multibaksetListWrapper.getAttribute('style') === 'display: none;') {
					this.multibaksetListWrapper.style = 'display: block;';
				} else {
					this.multibaksetListWrapper.style = 'display: none;';
				}

			}.bind(this));
			this.initCheckboxes(this.getEntities(this.getCacheNode(this.ids.basketRoot), 'baseket-item-checkbox'));
		},

		initCheckboxes: function (checkBoxses) {
			Array.prototype.slice.call(checkBoxses).forEach(function(i) {
				i.addEventListener('click', function() {
					const state = i.querySelector('svg');
					const id = i.id.replace('baseket-item-checkbox-', '');
					const item = this.items[id];
					if (!Array.isArray(this.multibasket.instance.basketSelectedAitems)) {
						return;
					}

					if (state.getAttribute('style') === 'display: none;') {
						state.setAttribute('style', 'display: block;');
						let addToSelect = true;
						this.multibasket.instance.basketSelectedAitems.forEach(elem => {
							addToSelect = elem.ID !== item.ID;
						});
						if (addToSelect) {
						this.multibasket.instance.basketSelectedAitems.push({
							ID: item.ID,
							NAME: item.NAME,
						...
						this.multibasket.instance.basketSelectedAitems = this.multibasket.instance.basketSelectedAitems
							.filter(function(i) {
								return i.ID !== item.ID;
							});
					}

					if (this.multibasket.instance.basketSelectedAitems.length === 0) {
						this.multibaksetListWrapper.style = 'display: none;';
					}

					if (this.multibasket.instance.basketSelectedAitems.length > 0) {
						this.multibaksetTitle.classList.add('multibasket__title-active');
					} else {
						this.multibaksetTitle.classList.remove('multibasket__title-active');
					}
				}.bind(this));
			}.bind(this));
		}
