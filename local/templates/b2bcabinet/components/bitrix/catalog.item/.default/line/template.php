...
$usePrivatePrice = Bitrix\Main\Loader::includeModule( "sotbit.privateprice") && \Bitrix\Main\Config\Option::get("sotbit.privateprice", "MODULE_STATUS", 0);
        ...
        if($countOffers > 1 && $i == 1)
        {
            ...
            */?>
            <tr role="row" class="offer-footer">
                ...
                <td colspan="<?=(count($arResult['TABLE_HEADER']) + count($arResult['TABLE_HEADER']['PRICES']) + 1 + ($usePrivatePrice ? 1 : 0))?>">
                    <div class="td-content">
                        <div class="td-content-item">
    ...
    foreach ($arResult['TABLE_HEADER'] as $code => $res)
    {
        switch ($code)
        {
            ...
            case 'PRICES':
                ...
                if ($usePrivatePrice) {
                    if($haveOffers && $i > 0) {
                        $privatePrice = SotbitPrivatePriceMain::setPlaceholder($item['OFFERS'][$i][$arParams['SOTBIT_PRIVATE_PRICE_PRODUCT_UNIQUE_KEY']], "");
                        $privatePriceValue = SotbitPrivatePriceMain::setPlaceholderValue($item['OFFERS'][$i][$arParams['SOTBIT_PRIVATE_PRICE_PRODUCT_UNIQUE_KEY']], "");

                    } elseif (!$haveOffers) {
                        $privatePrice = SotbitPrivatePriceMain::setPlaceholder($item[$arParams['SOTBIT_PRIVATE_PRICE_PRODUCT_UNIQUE_KEY']], '');
                        $privatePriceValue = SotbitPrivatePriceMain::setPlaceholderValue($item[$arParams['SOTBIT_PRIVATE_PRICE_PRODUCT_UNIQUE_KEY']], '');
                    }
                    echo "<td class='js-price' ".
                        "data-price_name='INDIVIDUAL' ".
                        "data-price_code='INDIVIDUAL'". " data-price_value='". $privatePriceValue ."'".
                        ">".
                        $privatePrice . "</td>";
                }
            ...
            case 'QUANTITY':
                ...
                {
                    $maxQnt = '';
                    if($item["CHECK_QUANTITY"] && $item["CATALOG_QUANTITY"]){
                        $maxQnt = $item["CATALOG_QUANTITY"];
                    }
                    ...
                    if( is_array($item['OFFERS'][$i]) && !empty($item['OFFERS'][$i]))
                    {
                        ...
                        if($item['OFFERS'][$i]["CHECK_QUANTITY"] && $item['OFFERS'][$i]["CATALOG_QUANTITY"]){
                            $maxQnt = $item['OFFERS'][$i]["CATALOG_QUANTITY"];
                        }
                    }
                    else
                    {
                    ...
                    echo "<td>".
                        ( !empty($quantity)
                            ...
                            "" ).
                        '<script>
                                    $(".touchspin-empty[data-id='. $itemId .']").TouchSpin({
                                        min: 0,
                                        max: '. ($maxQnt ?  $maxQnt : 10000000 ).',
