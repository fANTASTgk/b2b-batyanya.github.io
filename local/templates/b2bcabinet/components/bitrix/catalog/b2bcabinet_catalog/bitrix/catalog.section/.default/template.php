...
$sectionIds = [
    'BTN_EXCEL_OUT' =>  $obName . '_excel-out'
];
...
?>
<div class="index-blank__table-container">
    <!-- table_header -->
    <div class="table_header">
        <!-- total products -->
        <div id="block_filter_js" class="block_form_filter blank_filter"
             data-site-url="<?= $APPLICATION->GetCurPage() ?>"
    ...
    <? if (!empty($arResult['ITEMS'])): ?>
        <div class="dataTables_wrapper no-footer bx-<?= $arParams['TEMPLATE_THEME'] ?>"
            ...
            <div class="table-responsive">
                ...
                <div class="datatable-scroll">
                    ...
                    <div class="index_blank-thead_fixed-wrapper">
                        <div class="index_blank-thead_fixed">
                        </div>
                    </div>
                    <div class="anchor_header"></div>
                    <!-- table header -->
                    <table class="table dataTable no-footer">
                        <thead class="index_blank-thead">
                        <tr role="row">
                            <th></th>
                            ...
                            foreach ($arParams['TABLE_HEADER'] as $item) {
                                if (is_array($item))
                                    ...
                                    } else if (stristr($item, $arParams['TABLE_HEADER']['QUANTITY'])) {
                            ...
                            ?>
                            <?if (Bitrix\Main\Loader::includeModule( "sotbit.privateprice") && Option::get("sotbit.privateprice", "MODULE_STATUS", 0)):?>
                            <?
                                ...
                                ?>
                                <th>
                                    <div class="thead-item"><?=Loc::getMessage("CT_BCS_CATALOG_MESS_PRIVATE_PRICE_TITLE")?></div>
                                </th>
                            <?endif;?>
                        </tr>
                        </thead>
    ...
    <div>
        ...
        <div class="row-under-modifications row-under-modifications-fixed">
            ...
            <button type="button"
                    class="btn btn-light add_to_cart btn_b2b"
                    data-toggle="modal"
                    data-target="#modal_add_to_bascket"
                <?= $arParams["SHOW_BTN_ADD_TO_BASKET"] != "Y" ? "disabled" : "" ?>
            >
...
<div id="modal_excel-import" class="modal fade" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><?= Loc::getMessage('IMPORT_EXCEL') ?></h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>

            <div class="modal-body">
                <div class="modal_excel-import_success">
...
<script>
    BX.message({
        ...
        TITLE_IMPORT_SUCCESSFUL: '<?=GetMessageJS('IMPORT_EXCEL_SUCCESS')?>',
    });
    ...
    } else if (!empty($GLOBALS[$arParams['FILTER_NAME']]) && is_array($GLOBALS[$arParams['FILTER_NAME']])) {
        $filterProps = $GLOBALS[$arParams['FILTER_NAME']];
    } else if (!empty($GLOBALS["sotbitFilterResult"]['ITEMS']['SECTION_ID']['VALUES']) && is_array($GLOBALS["sotbitFilterResult"]['ITEMS']['SECTION_ID']['VALUES'])) {
    ...
    const catalogSection = new CatalogSection('.index-blank__table-container').init();
</script>
