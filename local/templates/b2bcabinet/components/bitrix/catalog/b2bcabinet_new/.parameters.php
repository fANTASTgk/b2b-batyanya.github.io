...
if($arCurrentValues['IBLOCK_TYPE'] == '={Config::get("CATALOG_IBLOCK_TYPE")}' && !empty($_REQUEST['src_site']))
    $arCurrentValues['IBLOCK_TYPE'] = Config::get("CATALOG_IBLOCK_TYPE", $_REQUEST['src_site']);

if($arCurrentValues['IBLOCK_ID'] == '={Config::get("CATALOG_IBLOCK_ID")}' && !empty($_REQUEST['src_site']))
    $arCurrentValues['IBLOCK_ID'] = Config::get("CATALOG_IBLOCK_ID", $_REQUEST['src_site']);
...
$arTemplateParameters = array(
    "IBLOCK_TYPE" => array(
        "PARENT" => "BASE",
        "NAME" =>  GetMessage('CPT_BC_IBLOCK_TYPE'),
        "TYPE" => "LIST",
        "VALUES" => $arIBlockType,
        "REFRESH" => "Y",
        "ADDITIONAL_VALUES" => "Y",
...
$arTemplateParameters['PROPERTY_ARTICLE'] = array(
    'PARENT' => 'SEARCH_SETTINGS',
    'NAME' => GetMessage("CP_BC_TPL_PROPERTY_ARTICLE"),
    "TYPE" => "STRING",
...
if ($iblockExists)
{
    ...
    $arTemplateParameters['LIST_SHOW_MEASURE_RATIO'] = array(
        'PARENT' => 'LIST_SETTINGS',
        'NAME' => GetMessage('CP_BC_TPL_MEASURE_RATIO'),
        'TYPE' => 'CHECKBOX',
        'MULTIPLE' => 'N',
        'REFRESH' => 'N',
        'DEFAULT' => 'Y'
	...
	if ($boolSKU)
	{
        ...
        // Property offers ARTICLE for detail page
        if(!empty($arAllOfferPropList)){
            $arTemplateParameters['ARTICLE_PROPERTY_OFFERS'] = array(
                'PARENT' => 'VISUAL',
                'NAME' => GetMessage('CP_BC_TPL_DETAIL_MAIN_ARTICLE_PROPERTY_OFFERS'),
                'TYPE' => 'LIST',
                'MULTIPLE' => 'N',
                'SIZE' => (count($arAllOfferPropList) > 5 ? 8 : 3),
	...
	if (!empty($showedProperties))
	{
        ...
        // Property ARTICLE for detail page
        $arTemplateParameters['ARTICLE_PROPERTY'] = array(
            'PARENT' => 'VISUAL',
            'NAME' => GetMessage('CP_BC_TPL_DETAIL_MAIN_ARTICLE_PROPERTY'),
            'TYPE' => 'LIST',
            'MULTIPLE' => 'N',
        ...
        // Property BRAND for detail page
        $arTemplateParameters['DETAIL_MAIN_BRAND_PROPERTY'] = array(
            'PARENT' => 'DETAIL_SETTINGS',
            'NAME' => GetMessage('CP_BC_TPL_DETAIL_MAIN_BRAND_PROPERTY'),
            'TYPE' => 'LIST',
            'MULTIPLE' => 'N',
            'SIZE' => (count($selected) > 5 ? 8 : 3),
            'VALUES' => $selected
        ...
        // Property FILES for detail page
        $arTemplateParameters['DETAIL_MAIN_FILES_PROPERTY'] = array(
            'PARENT' => 'DETAIL_SETTINGS',
            'NAME' => GetMessage('CP_BC_TPL_DETAIL_MAIN_FILES_PROPERTY'),
            'TYPE' => 'LIST',
            'MULTIPLE' => 'N',
            'SIZE' => (count($selected) > 5 ? 8 : 3),
            'VALUES' => $selected
...
// Settigs for sotbit:crosssell
if(Loader::includeModule('sotbit.crosssell') && \Bitrix\Main\Config\Option::get("sotbit.crosssell", 'sotbit.crosssell_INC_MODULE', '', $siteId) === 'Y') {
	$sectionMode = array(
		"N" => GetMessage("CP_BC_TPL_DETAIL_SECTION_MODE_N"),
		"Y" => GetMessage("CP_BC_TPL_DETAIL_SECTION_MODE_Y"),
	);
	...
	$arRes = array('s0' => GetMessage("CP_BC_TPL_DETAIL_NOT_CATEGORIES"));
	while ($ar = $arQuery->fetch()) {
		$arRes['e' . $ar['ID']] = ' . ' . $ar['NAME'];
	}

	$arTemplateParameters['DETAIL_CROSSSELL_STATUS'] = [
		'PARENT' => 'DETAIL_SETTINGS',
		'NAME' => GetMessage('CP_BC_TPL_DETAIL_CROSSSELL_STATUS'),
		'TYPE' => 'CHECKBOX',
		'DEFAULT' => 'N',
		'REFRESH' => 'Y',
		'DEFAULT' => 'N'
	...
	if ($arCurrentValues['DETAIL_CROSSSELL_STATUS'] === 'Y') {
		$arTemplateParameters['DETAIL_CROSSSELL_SECTION_MODE'] = [
			'PARENT' => 'DETAIL_SETTINGS',
			'NAME' => GetMessage('CP_BC_TPL_DETAIL_CROSSSELL_SECTION_MODE'),
			'TYPE' => 'LIST',
			'REFRESH' => 'Y',
			'VALUE' => 'Y',
			'VALUES' => $sectionMode,
		...
		if ($arCurrentValues['DETAIL_CROSSSELL_SECTION_MODE'] === 'Y') {
			$arTemplateParameters['DETAIL_CROSSSELL_LIST'] = array(
				'PARENT' => 'DETAIL_SETTINGS',
				'NAME' => GetMessage('CP_BC_TPL_DETAIL_CROSSSELL_LIST'),
				'TYPE' => 'LIST',
				'VALUES' => $arRes,
				'MULTIPLE' => 'Y',
				'REFRESH' => 'N',
