BX.saleOrderAjax = { // bad solution, actually, a singleton at the page
	...
	init: function(options)
	{
		...
		BX.proxy('eventForRadiabattonLabele', this);

	},

	eventForRadiabattonLabele: function () {

		person_type_wripper = this.controls.scope
			.querySelector('[data-entity="person_type_wripper"]');

		if (!person_type_wripper) {
			return;
		}

		Array.prototype.slice
			.call(person_type_wripper.querySelectorAll('.form-check-label'))
			.forEach(function(i) {
				var btn = i.querySelector('input');
	...
	getInputByPropId: function(propId){
		if(this.properties[propId].input) {
	...
	getAltLocPropByRealLocProp: function(propId){
		if(this.properties[propId].altLocationPropId)
