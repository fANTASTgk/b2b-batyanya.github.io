    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card" id="card">
                ...
                <?
                if (isset($arResult["COMPANY_PROPS_ERROR"]) && $arResult["COMPANY_PROPS_ERROR"] == "Y") {
                    ?>
                    <div style="margin-left: 20px;">
                        <p>
                            <?
                            echo ShowError(Loc::getMessage('SOA_NEED_ADD_PROPS_COMPANY'));
                            ?></br><?
                            echo Loc::getMessage(
                                'SOA_LINK_ADD_PROPS_COMPANY',
                                array(
                                    "#EDIT_COMPANY_LINK#" => SITE_DIR . (Option::get('sotbit.b2bcabinet',
                                            'method_install', '', SITE_ID) == 'AS_TEMPLATE' ?
                                            'b2bcabinet/' :
                                            '') . '/personal/companies/add.php?EDIT_ID=' . $_SESSION["AUTH_COMPANY_CURRENT_ID"]
                                )
                            );
                            ?>
                        </p>
                ...
                if (!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "N") {
                        ...
                        <? if ($_POST["is_ajax_post"] != "Y")
                    {
                        ...
                        <form action="<?= $APPLICATION->GetCurPage(); ?>" method="POST" name="ORDER_FORM"
                              id="ORDER_FORM" enctype="multipart/form-data" autocomplete="off">
                        ...
                        <div
                            class="checkout_form-individual section-order-props"
                            class="checkout_form-individual section-order-props"
                            style="order: <?=$arParams["LOCATION_OF_PROPERTIES"] == "BEFORE" ? -1 : 0?>"
                        >
                        ...
                        <? if ($_POST["is_ajax_post"] != "Y")
                    {
                        ...
                        {
                        ?>
                        <div style="display:none;"><? $APPLICATION->IncludeComponent("bitrix:sale.ajax.delivery.calculator",
            ...
            <div class="modal-footer popup-window-buttons">
                <div class="popup-window-buttons_btns">
                    <button type="button" class="btn btn-link"
                            data-dismiss="modal"><?= GetMessage('SOA_POPUP_CANCEL') ?></button>
                    <button
                            id="crmOk"
                            name="crmOk"
                            type="button"
...
<script type="text/javascript">
    ...
    function submitForm(val) {
        ...
        <?endif?>
        /*if (val == 'Y') {
            const companyLocation = document.querySelector('input[name="COMPANY_LOCATION"]');

            if (companyLocation) {
                document.querySelector('input[name="ORDER_PROP_' + companyLocation.getAttribute("data-property-id") + '"]').value = companyLocation.value;
            }
        }*/
    ...
    function ajaxResult(res) {
        var orderForm = BX('ORDER_FORM');
        UpdateEventForRadiabattonLabele();
        try {
    ...
    function UpdateEventForRadiabattonLabele() {
        setTimeout(function () {
            BX.saleOrderAjax.eventForRadiabattonLabele()
        }, 250)
    }
