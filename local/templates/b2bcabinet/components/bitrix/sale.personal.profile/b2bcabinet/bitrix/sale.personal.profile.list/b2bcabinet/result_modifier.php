...
if(Bitrix\Main\Loader::includeModule('sale') && !empty($arParams['BUYER_PERSONAL_TYPE']))
{
    ...
    if (is_array($userProfiles))
        $arResult['TOTAL_COUNT'] = count($userProfiles);
    ...
    if(!empty($filterData)){
        ...
        foreach ($arResult['ROWS'] as $key=>$row){
            foreach ($filter as $code => $value){
                ...
                if (strpos(mb_strtolower(str_replace($repSymbol, '', $row['data'][$code])), mb_strtolower(str_replace($repSymbol, '', $value))) === false) {
                    $saveResult = false;
                    break;
                }
                else{
