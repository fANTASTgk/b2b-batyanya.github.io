...
export const Catalog = () => {
    ...
    return <div className="upselling__catalog-list"
        onMouseMove={e => {
            if (e.target.classList.contains('catalog-list__name')) {
                setToolTip(() => ({
                ...
                setToolTip(() => ({
                    text: '', top: e.clientY, right: window.innerWidth - e.clientX, width: 0,
                }))
            }
        }}
    >
        <Tooltibp text={toolTip.text} top={toolTip.top} right={toolTip.right} width={toolTip.width} />
