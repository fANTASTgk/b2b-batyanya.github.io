...
if ($arResult['USER_PROPERTIES']['SHOW'] === 'Y') {
    $usersFilds = [];
    foreach ($arResult['USER_PROPERTIES']['DATA'] as $key => $value) {
        $usersFilds[$key] = [
            "arUserField" => ["USER_TYPE" => "profile_string"],
            "TEMPLATE" => 'input',
            'TYPE' => $value['USER_TYPE_ID'],
            'NAME' => $key,
            'ATTR' => [$value['EDIT_IN_LIST'] !== 'Y' ? 'readonly' : ''],
            'skip_manager' => 'Y',
            'VALUE' => $value['VALUE'],
            'LABEL' => $value['EDIT_FORM_LABEL'],
            'PLACEHOLDER' => $value['HELP_MESSAGE'],
        ];
    }
    $usersFilds['SUBMIT_BUTTON'] = [
        'NAME' => 'SUBMIT_BUTTON',
        "arUserField" => ["USER_TYPE" => "submit_button"],
    ];
    $arResult['USER_PROPERTIES']['SHOW_DATA'] = $usersFilds;
}
