...
<?else:?>
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls <? if($arResult['MODE_CLASSES'] <> ''): ?> <?= $arResult['MODE_CLASSES'] ?><? endif?>">
		...
		<div class="<?=$dropDownBlock?> bx-ui-sls-input-block form-control-feedback form-control-feedback-start">
			...
			<input type="text" autocomplete="off" name="<?=$arParams['INPUT_NAME']?>" value="<?=$arResult['VALUE']?>" class="form-control dropdown-field" placeholder="<?=Loc::getMessage('SALE_SLS_INPUT_SOME')?> ..." />
