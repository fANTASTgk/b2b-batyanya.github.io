...
if($idBuyers)
{
    ...
    if ($db_sales->arResult) {
        $arResult["USER_ORDERS_COUNT"] = count($db_sales->arResult);
    }
	...
	if($buyers)
	{
		foreach ($buyers as $id => $v)
		{
			if($v['INN'] == $org['VALUE'])
			{
				$name = $v['ORG'];
				$name .= ($v['INN'])?' ('.$v['INN'].')':'';
				$orgs[$org['ORDER_ID']] = '<a href="'. $methodIstall .'personal/buyer/profile_detail.php?ID='. $id .'">'. $name .'</a>';
			}
		}
	}
}
...
foreach($arResult['ORDERS'] as $arOrder)
{
	...
	$aActions = [
		["TEXT" => Loc::getMessage('SPOL_MORE_ABOUT_ORDER'), "ONCLICK" => "location.assign('".$urlToDitail."')" ],
	];
	...
	$arResult['ROWS'][] = array(
		'data' =>array_merge($arOrder['ORDER'], array(
            "ACCOUNT_NUMBER" => Loc::getMessage('SPOL_ORDER_NUMBER', array('#ID#' => $arOrder['ORDER']['ACCOUNT_NUMBER'])),
			...
			'PAYED' => Loc::getMessage('SPOL_'.($arOrder["ORDER"]["PAYED"] == "Y" ? 'YES' : 'NO')),
			'PAY_SYSTEM_ID' => $arOrder["ORDER"]["PAY_SYSTEM_ID"],
...
{
	$arAction = array(
		'repeat' => array("TEXT"=>Loc::getMessage('SPOL_REPEAT_ORDER'), "ONCLICK"=>"location.assign('".$arOrder['ORDER']["URL_TO_COPY"]."')"),
		'cancel' => array("TEXT"=>Loc::getMessage('SPOL_CANCEL_ORDER'), "ONCLICK"=>"if(confirm('".Loc::getMessage('SPOL_CONFIRM_DEL_ORDER')."')) window.location='".$arOrder['ORDER']["URL_TO_CANCEL"]."';"),
	);
