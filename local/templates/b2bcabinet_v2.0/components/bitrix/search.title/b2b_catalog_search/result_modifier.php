...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	while($arElement = $rsElements->Fetch())
	{
		...
		if ($arElement["CATALOG_TYPE"] == ProductTable::TYPE_SKU) {
			...
			$arFilter = ['IBLOCK_ID' => $arElement['IBLOCK_ID'], "IBLOCK_ACTIVE" => "Y",];
			$arOrder = Array();
			...
			foreach ($arOffers as $offer) {
				if (empty($arElement["PRICES"]) || end($arElement["PRICES"])["DISCOUNT_VALUE"] > $offer["MIN_PRICE"]["DISCOUNT_VALUE"]) {
					$arElement["PRICES"]["MIN_PRICE"] = $offer["MIN_PRICE"];
					$arElement["CATALOG_MEASURE_RATIO"] = $offer["CATALOG_MEASURE_RATIO"];
				}
			}
			
			if ($arElement["CATALOG_MEASURE_RATIO"]) {
				foreach($arElement["PRICES"] as &$price) {
					$price["PRINT_VALUE"] = CCurrencyLang::CurrencyFormat(
						$price["VALUE"] * $arElement["CATALOG_MEASURE_RATIO"],
						$price["CURRENCY"]
					);
					$price["PRINT_DISCOUNT_VALUE"] = CCurrencyLang::CurrencyFormat(
						$price["DISCOUNT_VALUE"] * $arElement["CATALOG_MEASURE_RATIO"],
						$price["CURRENCY"]
					);
				}
			}
	...
	$arProductIds = array_keys($arResult["ELEMENTS"]);

    $dbRatio = \Bitrix\Catalog\MeasureRatioTable::getList([
        'filter' => ['PRODUCT_ID' => $arProductIds],
    ]);
	...
	foreach ($arResult["RATIO"] as $id => $item) {
		if (in_array($id, $arProductIds)) {
			foreach ($arResult["ELEMENTS"][$id]["PRICES"] as &$price) {
				$price["PRINT_DISCOUNT_VALUE"] = CCurrencyLang::CurrencyFormat(
					$price["DISCOUNT_VALUE"] * $item["RATIO"],
					$price["CURRENCY"]
				);
				$price["PRINT_VALUE"] = CCurrencyLang::CurrencyFormat(
					$price["VALUE"] * $item["RATIO"],
					$price["CURRENCY"]
				);
			}
		}
...
if ($arResult["ELEMENTS"] && 
	...
	Option::get('sotbit.privateprice', 'MODULE_STATUS', 0) &&
    ...
    $productsHl = SotbitPrivatePriceMain::makeMainCheckFields($arProductIds, $params);

    if ($productsHl) {
        $arResult["PRODUCT_PRIVATE_PRICE"] = [];
        foreach ($productsHl as $id => $item) {
            $arResult["PRODUCT_PRIVATE_PRICE"][$id] = CurrencyFormat(CCurrencyRates::ConvertCurrency($arResult['RATIO'][$id] ? $arResult['RATIO'][$id]['RATIO'] * $item[$params['PRICE_COLUMN']] : $item[$params['PRICE_COLUMN']], $item[$params['CURRENCY_FORMAT']] ?: $item['PRICE_PRIVATE_CURRENCY'], $item['PRICE_PRIVATE_CURRENCY']), $item['PRICE_PRIVATE_CURRENCY']);
        }
    }
}
