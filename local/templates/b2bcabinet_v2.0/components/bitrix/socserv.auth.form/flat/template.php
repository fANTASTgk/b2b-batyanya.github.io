...
{
	const content = BX("bx_socserv_form_"+id);
	if(content)
	{
		const popup = BX.PopupWindowManager.create("socServPopup"+id, BX("bx_socserv_icon_"+id), {
			autoHide: true,
			closeByEsc: true,
			angle: {offset: 40},
			content: content,
		...
		const input = BX.findChild(content, {'tag':'input', 'attribute':{'type':'text'}}, true);
		if(input)
		{
		...
		const button = BX.findChild(content, {'tag':'input', 'attribute':{'type':'submit'}}, true);
		if(button)
		{
...
?>
	...
	<?if($service["ONCLICK"] == '' && $service["FORM_HTML"] <> ''):?>
			<div id="bx_socserv_form_<?=$service["ID"]?>" class="bx-authform-social-popup">
				<form class="d-flex align-items-center " action="<?=$arParams["AUTH_URL"]?>" method="post">
					<?=$service["FORM_HTML"]?>
					<?=$hiddens?>
