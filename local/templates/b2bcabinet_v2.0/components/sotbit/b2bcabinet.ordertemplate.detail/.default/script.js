...
function removeTemplate() {
    ...
    request.then(function (response) {
        BX.closeWait();
        document.querySelector('#modal_popup-order-remove [data-bs-dismiss="modal"]').click();
...
function toggleCompanyList() {
    console.log(document.querySelector('#company-list + .select2').classList.contains('select2-container--open'));
    if (document.querySelector('#company-list + .select2').classList.contains('select2-container--open')) {
        $('[name^=\'COMPANY\']').select2('close')
    } else {
        $('[name^=\'COMPANY\']').select2('open')
    }
}